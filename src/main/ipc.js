...
export function registerIpcHandlers() {
  ...
  // Waits for the one-time official-plugin download (plugins/migration.js),
  // so those plugins load this session.
  ipcMain.handle('plugins:list', async () => {
    await pluginMigrationSettled()
    return getLoadablePlugins().map((p) => ({ id: p.id, manifest: p.manifest }))
  })
