...
const MAX_BUFFER_CLIP_SECONDS = 600
...
// cross-directory-import reason as MAX_BUFFER_CLIP_SECONDS above.
...
function defaultVolumeEnvelope() {
  return {
    enabled: false,
    points: [
      { position: 0, gain: 1 },
...
const FLUC_DEFAULT_TIMING = { changeMinSeconds: 6, changeMaxSeconds: 14, transitionSeconds: 8 }
...
const NEUTRAL_WHOLE_MIX = { highpassHz: 0, lowpassHz: 20000, gainDb: 0, fadeInSeconds: 0, echoDelayMs: 0, echoDecay: 0, eq: [] }
...
const NEUTRAL_GROUP_FILTERS = { highpassHz: 0, lowpassHz: 20000, gainDb: 0, echoDelayMs: 0, echoDecay: 0, occlusion: 0, eq: [] }
...
function mixFluctuationMarkup(prefix) {
...
// values.
function canonicalMixFilters(obj, { includeFadeIn }) {
  const o = obj ?? {}
...
function requestAnalyser(kind, presetId, groupId) {
...
const LEVEL_HISTORY_LENGTH = 600

function pushLevel(history, level) {
  history.push(level)
...
function drawLevelHistory(canvas, history) {
...
${mixFluctuationMarkup('group')}
        ...
        <div id="editor-panel" class="editor-panel hidden">
          ...
          <div id="editor-crossfade-section" class="editor-crossfade">
            ...
            <p class="editor-crossfade-hint">Blends the seam so the loop isn't audible. Longer helps busy, textured audio (rain, crowds) — 0 disables blending. Automatically capped for very short loops.</p>
            <label class="editor-reverse-toggle">
...
${shotAxisMarkup('editor-schedule-pan', 'Pan', '(a random left/right position every trigger)')}
  ...
  tickPreviewPlayhead() {
    ...
    if (this.previewMode === 'saved') {
      ...
      const { loopStart } = this.loopEditorController.getLoopPoints()
      this.loopEditorController.setPlayhead(loopStart + active.audioEl.currentTime)
    ...
    const seekFromClientX = (clientX) => {
      ...
      if (this.previewMode === 'saved') {
        ...
        if (this.loopEditorController) {
          const { loopStart } = this.loopEditorController.getLoopPoints()
          this.loopEditorController.setPlayhead(loopStart + t)
        }
      } else {
        if (!this.loopEditorController) return
        const { loopStart, loopEnd } = this.loopEditorController.getLoopPoints()
